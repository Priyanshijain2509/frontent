        ...
        <>
          ...
          <Link href={`/users/${userId}/projects/${projectId}/news/new}`}>
            <svg className="h-5 w-5 text-green-600 inline-block" viewBox="0 0 24 24"
              ...
              "round" stroke-linejoin="round"> <rect x="3" y="3" width="18" height="18"
              rx="2" ry="2" />  <line x1="12" y1="8" x2="12" y2="16" />
              <line x1="8" y1="12" x2="16" y2="12" />
            </svg> Add News
          </Link>
          {news.map(newsItem => (
            <Link key={newsItem.id} href={
              `/users/${userId}/projects/${projectId}/news/${newsItem.id}`}>
              <div className='news-css'>
                <h3>{newsItem.news_title}</h3>
                <p>Added by {newsItem.news_added_by}, {timeDifference(new Date(),
                new Date(newsItem.created_at))}</p>
              </div>
            </Link>
          ))}
        </>
